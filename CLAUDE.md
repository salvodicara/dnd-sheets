...
- Dynamic steps: a wizard can implement `_buildSteps()` (top-level on the config, not per-step) to compute its step array at `openWizard()` time. The engine shallow-clones the config with the returned steps: `Object.assign({}, config, {steps: config._buildSteps()})`. Use this when the number or content of steps depends on runtime state (e.g. level-up ASI step only at levels 4/8/12/16/19).
- To add a new entity type, add a new wizard config and a button that calls `openWizard('wizardName')`.
- Field hints render as floating tooltips via `<span class="help-icon" data-tip="...">?</span>` — see **Unified Tooltip Rule** below.

#### Unified Tooltip Rule
ALL tooltips app-wide use `<span class="help-icon" data-tip="...">?</span>` plus the JS floating tooltip system (`position:fixed`, `.floating-tip`). The JS system is event-delegated on `document` via `mouseover`/`mouseout` on `[data-tip]` elements.

- **Never** use HTML `title=` attributes for user-visible tooltips.
- **Never** use CSS `::after` pseudo-element tooltips.
- To update a tooltip dynamically in `_afterRender`, find the `.help-icon` span and set `element.dataset.tip = "..."`.
- The `data-tip` attribute may appear on any element (not just `.help-icon`). Example: ability score `<td data-tip="Base: 16 + 2 (Outlander)">`.
...
humanOriginFeat           string (optional) — second origin feat for Human characters
bgFeat                    string (optional) — background feat name (used for wizard prefill; not rendered directly)
```
...
```
ability              "CHA"|"INT"|"WIS"
preparedCaster       boolean  (Cleric/Druid/Paladin/Ranger/Wizard prepare from list; Bard/Sorcerer/Warlock do NOT)
focus                string — e.g. "Arcane focus", "Musical instrument"
preparedMax          number (omit if not preparedCaster) — max prepared spells shown in spell section header
saveDCOverride       number|null  — overrides auto-computed spell save DC (8 + PB + ability mod)
attackBonusOverride  number|null  — overrides auto-computed spell attack bonus (PB + ability mod)
```
...
components     {v:boolean, s:boolean, m:boolean, material:string} — spell components
...
damageOverride      string|null  (null = auto: die + stat mod; set to full expression e.g. "1d8+4")
...
title      string
emoji      string
subtitle   string (optional) — short one-line subtitle shown below the title
...
{type:"paragraph", text:"..."}
{type:"note",      text:"📌 ..."}
{type:"header",    text:"..."}  — bold section heading within a feature
...
deathSucc       number
deathFail       number
inspiration     boolean — Heroic Inspiration (DM-granted)
bardicInspiration     boolean — Bardic Inspiration (received from a Bard)
bardicInspirationDie  string — die type received: "d6"|"d8"|"d10"|"d12"
exhaustion      number (0–6) — D&D 2024 exhaustion level
notes           string
